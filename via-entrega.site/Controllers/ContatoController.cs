...
namespace Controllers
{
    ...
    public class ContatoController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            var newLista = lista.Where(x => x.Active &&
            x.PessoaId == Request.HttpContext.GetPessoaId())
                .Select(c => ContatoViewModel.MapearParaViewModel(c!)).ToList();
        ...
        public async Task<IActionResult> Save(ContatoViewModel contatoViewModel)
        {
            ...
            {
                var novoId = await _contatoService.CreateAsync(contatoViewModel.MapearParaEntidade(Request.HttpContext));
                if (novoId == null) return BadRequest("Não foi possível criar o contato.");
                return Ok(new { id = novoId });
            }
            else
            {
                var atualizado = await _contatoService.UpdateAsync(contatoViewModel.MapearParaEntidade(Request.HttpContext));
